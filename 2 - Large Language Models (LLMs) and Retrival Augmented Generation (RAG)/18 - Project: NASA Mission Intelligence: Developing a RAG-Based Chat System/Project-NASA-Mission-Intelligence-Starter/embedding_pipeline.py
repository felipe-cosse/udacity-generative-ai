...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        self.collection = self.chroma_client.get_or_create_collection(
            ...
            metadata={
                "description": "NASA mission text chunks with OpenAI embeddings",
        ...
        # TODO: Try to break at sentence boundaries
            if hard_end < text_length:
                ...
                for separator in ("\n\n", ". ", "! ", "? ", "\n"):
                    ...
                    if position != -1:
                        boundary_end = position + len(separator.rstrip(" "))
                        candidate_ends.append(boundary_end)
        ...
        # TODO: Create metadata for each chunk
            ...
            chunk_metadata.update({
                ...
                "overlap_with_previous": (
                    0 if not chunks else self.chunk_overlap
                ),
            })

            chunks.append((text[start:end], chunk_metadata))
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        mission = str(metadata.get("mission", "unknown"))
        source = str(metadata.get("source") or file_path.stem)
        chunk_index = int(metadata.get("chunk_index", 0))

        mission = "_".join(mission.strip().lower().split())
        source = "_".join(source.strip().lower().split())
        # TODO: Use mission, source, and chunk_index
        # Format: mission_source_chunk_0001
        chunk_number = chunk_index + 1
        return f"{mission}_{source}_chunk_{chunk_number:04d}"
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        update_mode = update_mode.strip().lower()
        ...
        if update_mode == "replace":
            ...
            if existing_ids:
                ...
                logger.info(
                    f"Deleted {len(existing_ids)} existing documents "
        ...
        for batch_start in range(0, len(documents), batch_size):
            ...
            for text, metadata in batch:
                ...
                exists = (
                    False
                    if update_mode == "replace"
                    else self.check_document_exists(doc_id)
                )
                ...
                if not embedding:
                    raise ValueError(
                        f"No embedding generated for document {doc_id}"
                    )
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        files_to_process = sorted(
            self.scan_text_files_only(base_path)
        )

        logger.info(
            f"Found {len(files_to_process)} text files to process"
        )
        ...
        # TODO: Update statistics
                ...
                logger.info(
                    ...
                    f"{document_stats['added']} added, "
                    f"{document_stats['updated']} updated, "
                    f"{document_stats['skipped']} skipped"
                )
        # TODO: Handle errors gracefully
            except Exception as error:
                stats["errors"] += 1
                mission_stats["errors"] += 1
    ...
    def query_collection(self, query_text: str, n_results: int = 5) -> Dict[str, Any]:
        ...
        if not query_embedding:
            raise ValueError("Failed to generate the query embedding")
